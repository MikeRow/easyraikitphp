<?php
	...
	*/
	...
	DEFINE("RAIN", "1000000000000000000000000"); // A string to concatenate to build 1Rai value
	...
	function americanu($number){
	
		return number_format($number,24,".",",");
	...
	function rb_call_method($method,$params = null){
		...
		if( isset($result["balance"]) ){ $result["balance_rai"] = americanu($result["balance"]/RAIN); }
		if( isset($result["weight"]) ){ $result["weight_rai"] = americanu($result["weight"]/RAIN); }
		if( isset($result["count"]) ){ $result["count_readable"] = americanu($result["count"]); }
